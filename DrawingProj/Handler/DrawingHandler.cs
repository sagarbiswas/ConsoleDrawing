...
using System.Linq;
using System.Text;
using DrawingProj.Interface;
using DrawingProj.Model;
using DrawingProj.Utility;

namespace DrawingProj.Controller
{
    public class DrawingHandler : IDraw
    {
        ...
        /// <summary>
        /// This method is used  to write is x,y corodinate if the space is blank.
        /// </summary>
        /// <param name="x">X point from left</param>
        /// <param name="y">Y point from top</param>
        public void WriteInConsoleXYLocation(int x,int y)
        {
            if (!LstConsoleContent.Any(t => t.x == (x + DefaultLeftPostion) && t.y == (y + DefaultTopPostion)))
            {
                WriteAtPosition(drawChar.HasValue ? drawChar.Value.ToString() : ".", x, y);
            }
        }
